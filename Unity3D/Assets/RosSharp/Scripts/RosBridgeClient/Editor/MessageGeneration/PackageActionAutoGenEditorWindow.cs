...
{
    public class PackageActionAutoGenEditorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            GUILayout.Label("Package actions auto generation", EditorStyles.boldLabel);
            ...
            if (GUILayout.Button("GENERATE!"))
            {
                ...
                else
                {
                    try
                    {
                        string[] files = Directory.GetFiles(Path.Combine(inPkgPath, "action"), "*.action");
                        if (files.Length == 0)
                        {
                            EditorUtility.DisplayDialog(
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < files.Length; i++)
                            {
                                ...
                                try
                                {
                                    warnings.AddRange(ActionAutoGen.GenerateSingleAction(file, outPkgPath, rosPackageName));
                                }
